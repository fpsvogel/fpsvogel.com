...
So the question I set out to answer was, *"How can I move the API calls to be **outside the request** that shows new content, while still sending the final results from the API calls back to the page?"*
...
Another option would be to use WebSockets (in Rails, see [Action Cable](https://guides.rubyonrails.org/action_cable_overview.html)). There are WebSockets setups for Roda out there (e.g. [1](https://github.com/socketry/roda-websockets), [2](https://github.com/nanne007/roda/blob/master/lib/roda/plugins/websockets.rb)), but they seem more complex than I need—after all, WebSockets enable two-way communication between the client and server, but **I only need to send messages *one-way*** from the server to the client.

My search for lightweight, asynchronous client-to-server communication led me to…
...
Then I added the `GET /next` route. [Here's that code](https://github.com/fpsvogel/wikistumble/blob/62bd341c378a4f7040ad8177443b5110a6b5088d/app/router.rb#L91-L138) as I first wrote it. (Eventually I replaced Roda's `streaming` plugin; more on that below.)

One gotcha is that connections need to be manually closed on the client side, or else the GET endpoint will keep being pinged even after the `<turbo-stream-source>` is no longer on the page. So I wrote [a Stimulus controller](https://github.com/fpsvogel/wikistumble/blob/4d5d48332a8dc27c278af9cfdaae4232c0866324/public/app.js#L16-L36) that closes each connection when its `<turbo-stream-source>` element is disconnected.

Following [the basics of Stimulus](https://stimulus.hotwired.dev/), each target element needs to have a target attribute:

```html
<turbo-stream-source src="/next" data-next-articles-target="stream"></turbo-stream-source>
```

And to the container of those elements, I added the Stimulus controller name:
...
Server-sent events allowed Wiki Stumble to return a response before the next article was done being fetched. Great! But **on its own, this isn't enough.** At best, in place of the old article, the user would see a placeholder with a loading spinner until the next article was loaded.
...
5. Meanwhile, the newly-added `<turbo-stream-source>` sends a GET request that fetches a new article. This takes a few seconds, but it's not a problem *as long as the buffer of next articles isn't empty*.
...
7. My Stimulus controller sees that the `<turbo-stream-source>` has been replaced, and proceeds to close the connection with the server.
...
2. If the user advanced through new articles faster than the buffer was filled, the app would stall and feel slow again. (See the *"as long as…"* in #5 above.)
3. Each connection was occupying a server thread, and since connections were open for several seconds at a time waiting for next articles, the app would slow down if more than a handful of people were using the app simultaneously.
