...
Recently I did an experiment: using the command-line querying interface, I got lists of my favorites from various genres and pasted the titles into an LLM, asking for similar recommendations. I got some pretty good suggestions.

Sure, you could get similar results by being a user of any one of a dozen AI-powered book tracking and recommendation platforms.

But that's the thing: I don't want to be locked into any one service, limited by their feature set, their UI, and their continued existence—or their capability (I would hope) to export my complete data when I need to jump ship.
